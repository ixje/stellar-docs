...
- Note: Sometimes account and shared state can merge when the contract scope is small
  - I.e. a contract account or a single NFT, where a new contract instance is generated for each account
...
- Owned contracts
  - Contract instances that have a clear owner
    - A contract account or a single NFT, where a new contract instance is generated for each account
    - Custom reserve backed assets (USDC, etc)
- Autonomous contracts
  - Contract instances that have no clear owner, or have a decentralized group of owners
